...
//            if (!empty($oauth_module->tokenParamName)) {
//                $query_param_auth['tokenParam'] = $oauth_module->tokenParamName;
//            }
//
//            $auth_behavior = $this->owner->getBehavior('authenticator');
//            $auth_behavior->class = CompositeAuth::className();
//            $auth_behavior->authMethods = [
//                ['class' => HttpBearerAuth::className()],
//                $query_param_auth,
//            ];

            $this->owner->attachBehavior('authenticator', [
                    ...
                    if (isset($rule['allow'])) {
                        if ($rule['allow'] && (empty($rule['roles']) || in_array('?', $rule['roles']))) {
                            ...
                            $is_public = true;
                                ...
                                (!$rule['allow'] && (empty($rule['roles']) || in_array('?', $rule['roles']))) || ($rule['allow'] && !empty($rule['roles']) && in_array('@', $rule['roles']))
                        ) {
